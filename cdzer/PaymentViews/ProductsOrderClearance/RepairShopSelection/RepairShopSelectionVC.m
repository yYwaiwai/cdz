...
@interface RepairShopSelectionVC () <UITableViewDelegate, UITableViewDataSource, UISearchBarWithReturnKeyDelegate, UIScrollViewDelegate, DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>

@property (nonatomic, weak) IBOutlet RSSVCViewFilterComponent *filterView;
...
- (void)componentSetting {
    @autoreleasepool {
        ...
        [[UINib nibWithNibName:@"RSSVCViewFilterComponent" bundle:nil] instantiateWithOwner:self options:nil];
        [self.filterView addSelfByFourMarginToSuperview:self.view];
        
//        [self setRightNavButtonWithTitleOrImage:@"地图列表" style:UIBarButtonItemStylePlain target:self action:nil titleColor:CDZColorOfDefaultSubColor isNeedToSet:YES];
...
- (void)getRepairShopCombinationList:(id)refreshView isReloadAll:(BOOL)isReloadAll {
    ...
    [APIsConnection.shareConnection maintenanceExpressAPIsGetMaintenanceClearanceShopSelectionListWithAccessToken:self.accessToken maintainItemList:self.selectedMaintainItemList coordinate:self.coordinate cityName:self.cityName pageNums:self.pageNums pageSizes:self.pageSizes mainFilterOption:@(self.filterView.currentFilterTypeIdx-1) filterOption:self.filterView.subTypeFilterSequence searchKeyword:self.searchBar.text brandID:autosData.brandID success:^(NSURLSessionDataTask *operation, id responseObject) {
        @strongify(self);
