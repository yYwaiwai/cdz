...
@property (nonatomic, strong) NSMutableArray <NSArray <AutosBrandDTO *> *> *mainTypeBrandSelectionList;//车品牌主选项据
@property (nonatomic, strong) NSArray <NSDictionary *> *mainTypeServiceSelectionList;//专修主选项数据
...
- (void)setToItemBrandFilterTypeOnlyWith:(SNSSLVFItemBrandType)itemBrandType {
    self.mainTypeCurrentSelection = SNSSLVFilterTypeOfSpecRepairOption;
    if (itemBrandType==SNSSLVFItemBrandTypeOfNone) itemBrandType = SNSSLVFItemBrandTypeOfTire;
    self.itemBrandListView.itemBrandType = itemBrandType;
}
...
- (void)setToItemSpecShopFilterTypeWithItemIdx:(NSIndexPath *)indexPath specShopServiceTypeList:(NSArray <NSDictionary *> *)specShopServiceTypeList {
    self.mainTypeCurrentSelection = SNSSLVFilterTypeOfSpecRepairOption;
    self.lastSelectedBtnTag = self.mainTypeBtnControlView.tag;
    self.wasFilterViewExpand = YES;
    self.mainTypeServiceSelectionList = specShopServiceTypeList;
    [self changeMainTypeSelection:self.mainTypeOfSpcRepairBtnControlView];
    [self.mainTypeTableView selectRowAtIndexPath:indexPath animated:NO scrollPosition:UITableViewScrollPositionMiddle];
    [self tableView:self.mainTypeTableView didSelectRowAtIndexPath:indexPath];
...
- (void)updateMainTypeStatus:(BOOL)mainTypeSelected andSubTypeStatus:(BOOL)subTypeSelected itemBrandTypeStatus:(BOOL)itemBrandTypeSelected {
    [self.mainTypeBtnControlView.subviews.lastObject.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([obj isKindOfClass:UILabel.class]) {
            ...
            NSString *title = @"全部类型";
            if (self.mainTypeCurrentSubSelection) {
                NSIndexPath *indexPath = self.mainTypeCurrentSubSelection;
                if (self.mainTypeCurrentSelection==SNSSLVFilterTypeOfBrandOption) {
                    AutosBrandDTO *brandDTO = [self.mainTypeBrandSelectionList[indexPath.section] objectAtIndex:indexPath.row];
                    title = brandDTO.brandName;
    ...
    [self.itemBrandTypeBtnControlView.subviews.lastObject.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:UILabel.class]) {
            UILabel *label = (UILabel *)obj;
            label.text = @"品牌";
            if (self.itemBrandName&&![self.itemBrandName isEqualToString:@""]) {
                label.text = self.itemBrandName;
            }
            label.highlighted = itemBrandTypeSelected;
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView==self.mainTypeTableView) {
        ...
        self.mainTypeCurrentSubSelection = indexPath;
        [self.subTypeTableView reloadData];
        self.subTypeCurrentSelection = [NSIndexPath indexPathForRow:0 inSection:0];
        [self.subTypeTableView selectRowAtIndexPath:self.subTypeCurrentSelection animated:NO scrollPosition:UITableViewScrollPositionNone];
        [self showFilterViewByBtns:self.mainTypeBtnControlView];
