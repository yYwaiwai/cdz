...
#define WK_IS_IPAD ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad)
#define WK_IS_LANDSCAPE ([UIApplication sharedApplication].statusBarOrientation == UIInterfaceOrientationLandscapeLeft || [UIApplication sharedApplication].statusBarOrientation == UIInterfaceOrientationLandscapeRight)

static char WKWebViewControllerKVOContext = 0;
...
- (void)commonInit
{
    self.webViewContentScale = 100;
    self.supportedWebNavigationTools = WKWebNavigationToolAll;
    self.supportedWebActions = WKWebActionAll;
    ...
    [self.webView addObserver:self forKeyPath:@"loading" options:NSKeyValueObservingOptionNew context:&WKWebViewControllerKVOContext];
...
- (UIBarButtonItem *)backwardBarItem
{
    if (!_backwardBarItem)
    {
        ...
        _backwardBarItem.accessibilityLabel = NSLocalizedStringFromTable(@"Backward", @"WKWebViewController", @"Accessibility label button title");
...
- (UIBarButtonItem *)forwardBarItem
{
    if (!_forwardBarItem)
    {
        ...
        _forwardBarItem.accessibilityLabel = NSLocalizedStringFromTable(@"Forward", @"WKWebViewController", @"Accessibility label button title");
...
- (UIBarButtonItem *)actionBarItem
{
    if (!_actionBarItem)
    {
        ...
        _actionBarItem.accessibilityLabel = NSLocalizedStringFromTable(@"Share", @"WKWebViewController", @"Accessibility label button title");
...
- (NSArray *)navigationToolItems
{
    ...
    if ((self.supportedWebNavigationTools & WKWebNavigationToolBackward) > 0 || self.supportsAllNavigationTools) {
        [items addObject:self.backwardBarItem];
    }
    
    if ((self.supportedWebNavigationTools & WKWebNavigationToolForward) > 0 || self.supportsAllNavigationTools) {
        if (!WK_IS_IPAD) [items addObject:flexibleSpace];
        [items addObject:self.forwardBarItem];
    }
    
    if ((self.supportedWebNavigationTools & WKWebNavigationToolStopReload) > 0 || self.supportsAllNavigationTools) {
        if (!WK_IS_IPAD) [items addObject:flexibleSpace];
...
{
    return (_supportedWebNavigationTools == WKWebNavigationToolAll) ? YES : NO;
}

- (UIImage *)backwardButtonImage
{
    if (!_backwardButtonImage) {
        _backwardButtonImage = [UIImage imageNamed:@"dzn_icn_toolbar_backward" inBundle:[NSBundle bundleForClass:[WKWebViewController class]] compatibleWithTraitCollection:nil];
    }
...
- (UIImage *)forwardButtonImage
{
    if (!_forwardButtonImage) {
        _forwardButtonImage = [UIImage imageNamed:@"dzn_icn_toolbar_forward" inBundle:[NSBundle bundleForClass:[WKWebViewController class]] compatibleWithTraitCollection:nil];
    }
...
- (UIImage *)reloadButtonImage
{
    if (!_reloadButtonImage) {
        _reloadButtonImage = [UIImage imageNamed:@"dzn_icn_toolbar_reload" inBundle:[NSBundle bundleForClass:[WKWebViewController class]] compatibleWithTraitCollection:nil];
    }
...
{
    if (!_stopButtonImage) {
        _stopButtonImage = [UIImage imageNamed:@"dzn_icn_toolbar_stop" inBundle:[NSBundle bundleForClass:[WKWebViewController class]] compatibleWithTraitCollection:nil];
    }
...
- (UIImage *)actionButtonImage
{
    if (!_actionButtonImage) {
        _actionButtonImage = [UIImage imageNamed:@"dzn_icn_toolbar_action" inBundle:[NSBundle bundleForClass:[WKWebViewController class]] compatibleWithTraitCollection:nil];
    }
...
- (NSArray *)applicationActivitiesForItem:(id)item
{
    ...
    if ((_supportedWebActions & WKWebActionCopyLink) > 0 || self.supportsAllActions) {
        [activities addObject:[DZNPolyActivity activityWithType:DZNPolyActivityTypeLink]];
    }
    if ((_supportedWebActions & WKWebActionOpenSafari) > 0 || self.supportsAllActions) {
        [activities addObject:[DZNPolyActivity activityWithType:DZNPolyActivityTypeSafari]];
    }
    if ((_supportedWebActions & WKWebActionOpenChrome) > 0 || self.supportsAllActions) {
        [activities addObject:[DZNPolyActivity activityWithType:DZNPolyActivityTypeChrome]];
    }
    if ((_supportedWebActions & WKWebActionOpenOperaMini) > 0 || self.supportsAllActions) {
        [activities addObject:[DZNPolyActivity activityWithType:DZNPolyActivityTypeOpera]];
    }
    if ((_supportedWebActions & WKWebActionOpenDolphin) > 0 || self.supportsAllActions) {
...
- (NSArray *)excludedActivityTypesForItem:(id)item
{
    ...
    if ((_supportedWebActions & WKSupportedWebActionshareLink) == 0) {
    ...
    if ((_supportedWebActions & WKWebActionReadLater) == 0 && [item isKindOfClass:[UIImage class]]) {
...
{
    return (_supportedWebActions == WKWebActionAll) ? YES : NO;
}
...
#pragma mark - WKWebViewController methods
...
- (void)showBackwardHistory:(UIGestureRecognizer *)sender
{
    ...
    [self presentHistoryControllerForTool:WKWebNavigationToolBackward fromView:sender.view];
}
...
- (void)showForwardHistory:(UIGestureRecognizer *)sender
{
    ...
    [self presentHistoryControllerForTool:WKWebNavigationToolForward fromView:sender.view];
}

- (void)presentHistoryControllerForTool:(WKWebNavigationTools)tool fromView:(UIView *)view
{
    UITableViewController *controller = [UITableViewController new];
    controller.title = NSLocalizedStringFromTable(@"History", @"WKWebViewController", nil);
    ...
    UIView *bar = WK_IS_IPAD ? self.navigationBar : self.toolbar;
...
- (void)configureToolBars
{
    ...
    if (self.hideBarsWithGestures) {
        [self.navigationBar addObserver:self forKeyPath:@"hidden" options:NSKeyValueObservingOptionNew context:&WKWebViewControllerKVOContext];
        [self.navigationBar addObserver:self forKeyPath:@"center" options:NSKeyValueObservingOptionNew context:&WKWebViewControllerKVOContext];
        [self.navigationBar addObserver:self forKeyPath:@"alpha" options:NSKeyValueObservingOptionNew context:&WKWebViewControllerKVOContext];
    }

    if (!WK_IS_IPAD && self.navigationController.toolbarHidden && self.toolbarItems.count > 0) {
...
- (void)updateStateBarItem
{
    ...
    self.stateBarItem.accessibilityLabel = NSLocalizedStringFromTable(self.webView.isLoading ? @"Stop" : @"Reload", @"WKWebViewController", @"Accessibility label button title");
...
- (void)webView:(DZNWebView *)webView didUpdateProgress:(CGFloat)progress
{
    ...
    if (progress>=1) {
        NSString *scaleHTML = [NSString stringWithFormat:@"document.getElementsByTagName('body')[0].style.webkitTextSizeAdjust='%lu%@\'", (unsigned long)self.webViewContentScale, @"%"];
...
{
    if (tableView.tag == WKWebNavigationToolBackward) {
        return self.webView.backForwardList.backList.count;
    }
    if (tableView.tag == WKWebNavigationToolForward) {
...
{
    if (context != &WKWebViewControllerKVOContext) {
...
{
    if (self.hideBarsWithGestures) {
        [self.navigationBar removeObserver:self forKeyPath:@"hidden" context:&WKWebViewControllerKVOContext];
        [self.navigationBar removeObserver:self forKeyPath:@"center" context:&WKWebViewControllerKVOContext];
        [self.navigationBar removeObserver:self forKeyPath:@"alpha" context:&WKWebViewControllerKVOContext];
    }
    [self.webView removeObserver:self forKeyPath:@"loading" context:&WKWebViewControllerKVOContext];
